...
class Tx_ContentLister_ItemsProcFunction {
    ...
    public function getSelectedTable($config){
        //Tx_Extbase_Utility_Debugger::var_dump($config);
        $flexFormArray = GeneralUtility::xml2array($config['row']['pi_flexform']);
        //t3lib_utility_Debug::debug($flexFormArray, "debug flexFormArray:" );
        if(! is_array($flexFormArray)){
            return null;
        }

        $table = $flexFormArray['data']['sDEF']['lDEF']['settings.flexform.select_table']['vDEF'];
        if($table){
    ...
    public function getCategories($config){
        ...
        $category = $flexFormArray['data']['Category']['lDEF']['settings.flexform.category']['vDEF'];
        $config['items'] = array();
